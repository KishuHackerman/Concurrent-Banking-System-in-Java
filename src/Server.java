...
public class Server extends Thread {
    ...
    public boolean processTransactions(Transactions trans)
    {
        ...
        while ((!Network.getClientConnectionStatus().equals("disconnected")))
        {
            ...
            if (!Network.getInBufferStatus().equals("empty"))
            {
                ...
                Network.transferOut(trans);   /* Transfer a completed transaction to output buffer */
    ...
    public double deposit(int i, double amount)
    {
        if (OSpa2driver.RUN_MODE == 1)
        {
            ...
            System.out.println("\n DEBUG : Server.deposit - " + "i " + i +
                    " Current balance " + curBalance + " Amount " + amount + " " + getServerThreadId());
    ...
    public void run()
    {
        ...
        System.out.println("\n Terminating server thread - " + getServerThreadId()
                + " Running time " + (serverEndTime - serverStartTime) + " milliseconds");
    }
}
